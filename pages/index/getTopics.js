...
function showData(data) {
  ...
  for (let i = 0, len = data.length; i < len; i++) {
    ...
    topic += `<div class="section-list-topic">
            <!-- 1.photo-->
            <a class="photo" href="../index/personalHomepage.html" class="photo"><img src="${data[i].author.avatar_url}" /></a>
            <!-- 2.num -->
            <span class="num">
...
function turnPage(opt) {
  ...
  let page = document.getElementById('click-btns');
  page.innerHTML = '';
  ...
  if (opt.currentPage > 2) {
    for (let i = opt.currentPage - 2; i <= opt.currentPage + 2; i++) {
  ...
  let block = nav.getElementsByTagName("a");
  let text = ['all', 'good', 'share', 'ask', 'job', 'dev']
  ...
  //点击翻页按钮，获取相应板块中页码数据
  let li = page.getElementsByTagName("li")
